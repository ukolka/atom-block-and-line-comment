...
export function activate() {
  atom.commands.add('atom-text-editor:not([mini])', {
    'block-and-line-comment:toggle': toggle
  });
}
...
function toggle(){
  ...
  if (monoline(editor.getGrammar().name)||(selections.length===1 && selectionIsEmpty(selections[0])))
    atom.commands.dispatch(editorView, 'editor:toggle-line-comments')
  else
    cblock(editor,editorView,buffer,selections)
  ...
  function monoline(lng){
    var lang=['LaTeX','TeX']
    return lang.indexOf(lng)!==-1
  }
}

function cblock(editor,editorView,buffer,selections) {
